...
class BatchTransferManager:
    def __init__(self, chain_config):
        ...
        # --- ENS (optional; keep legacy resolver for reverse lookups)
        self.ens = None
        try:
            if ENS is not None:
                self.ens = ENS.from_web3(self.w3)
            else:
                self.console.log("[yellow]python-ens not installed; ENS disabled[/yellow]")
        except Exception as e:
            self.console.log(f"[yellow]ENS init failed: {e}[/yellow]")
            self.ens = None
    ...
    def select_private_key_input_method(self):
        ...
        self.console.log(f"[green]Loaded {len(self.wallet_addresses)} wallet(s).")
    ...
    def prefetch_wallet_token_metadata(self) -> None:
        if not self.wallet_addresses or not self.token_addresses:
        ...
        try:
            result = self.web3h.multicall_fetch(
                ...
                spender=None,
                ens_names=getattr(self.web3h, "ens_names", []),
                use_multicall=True,
                want_balance=True,
                want_decimals=True,
        ...
        self.prefetched_ens_forward = ens_fwd_raw
    ...
    def reverse_ens(self, address: str) -> Optional[str]:
        if not self.ens:
            return None
        try:
            return self.ens.name(self.w3.to_checksum_address(address))
        except Exception:
            return None
    ...
    def run(self):
        ...
        for idx, wallet in enumerate(self.wallet_addresses, 1):
            wallet_cs = self._coerce_address_key(wallet)
            ens = self.prefetched_ens_reverse.get(wallet_cs)
            if ens:
                self.console.log(f"{idx:>2}. {wallet_cs} (ENS: {ens})")
            else:
        ...
        for idx, rcv in enumerate(self.receiver_addresses , 1):
            ...
            ens = self.prefetched_ens_reverse.get(rcv_cs)
            if ens:
        ...
        if mode.startswith("Same number"):
            if len(self.wallet_addresses) != len(self.receiver_addresses):
            ...
            if amt_mode.startswith("Enter fixed"):
                ...
                for s, r in zip(self.wallet_addresses, self.receiver_addresses):
        ...
        # ----- Multi-number (multi-to-many) -----
            ...
            if sub.startswith("Same amount"):
                ...
                if amt_mode.startswith("Enter fixed"):
                    ...
                    sender_remaining = {s: balances.get(self._coerce_address_key(s), 0) or 0 for s in self.wallet_addresses}
                    for r in self.receiver_addresses:
                        remaining = targets[r]
                        for s in self.wallet_addresses:
                            if remaining == 0:
                                break
                    ...
                    try:
                        ...
                        self.console.log("[red]Invalid percentage[/red]"); return
                    per_sender_amt = {s: int((balances.get(self._coerce_address_key(s), 0) or 0) * (pct_val / 100.0)) for s in self.wallet_addresses}
                ...
                if in_mode.startswith("Enter fixed"):
                    ...
                    try:
                        ...
                        self.console.log("[red]Invalid amount[/red]"); return
                    total_sum = per_sender_amt * len(self.wallet_addresses)
                else:
                    ...
                    try:
                        ...
                        self.console.log("[red]Invalid percentage[/red]"); return
                    per_sender_map = {s: int((balances.get(self._coerce_address_key(s), 0) or 0) * (pct_val / 100.0)) for s in self.wallet_addresses}
                ...
                if in_mode.startswith("Enter fixed"):
                    sender_remaining = {s: per_sender_amt for s in self.wallet_addresses}
                else:
        ...
        with progress:
            ...
            for (sender, receiver, amt_wei) in plan:
                try:
                    pk = self.wallet_private_keys[self.wallet_addresses.index(sender)]
                    if fixed_fees:
