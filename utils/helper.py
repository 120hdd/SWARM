...
class Web3Helper:
    ...
    def __init__(self, chain_config, console=None):
        ...
        self.ens_registry = self.w3.eth.contract(
            address=self.w3.to_checksum_address(getattr(self.cfg, "ENS_REGISTRY_ADDRESS", "0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e")),
            abi=json.loads(getattr(self.cfg, "ENS_REGISTRY_ABI")))
    ...
    def _reverse_node(self, address: str) -> bytes:
        addr_hex = self.w3.to_checksum_address(address)[2:].lower()
    ...
    def _aggregate3(self, calls: List[Tuple[str, bytes]], allow_failure: bool=True) -> List[Tuple[bool, bytes]]:
        """
        Execute Multicall3.aggregate3 using config MULTICALL3_ABI.
        Each call is (target, calldata). Returns [(success, returnData), ...].
        Fallback to single eth_call if needed.
        """
        try:
            call3 = [{"target": self.w3.to_checksum_address(t), "allowFailure": allow_failure, "callData": d} for t, d in calls]
            results = self.multicall.functions.aggregate3(call3).call()
            # results: list of dicts with 'success' and 'returnData'
    ...
    def batch_ens_reverse(self, wallets: List[str], use_multicall: bool=True) -> Dict[str, Optional[str]]:
        nodes = {self.w3.to_checksum_address(w): self._reverse_node(w) for w in wallets}
        # 1) resolver(node)
        if use_multicall:
            calls = [(self.ens_registry.address, self.ens_registry.encodeABI(fn_name="resolver", args=[node])) for node in nodes.values()]
            res = self._aggregate3(calls, allow_failure=True)
        else:
            res = []
            for node in nodes.values():
                try:
                    r = self.ens_registry.functions.resolver(node).call()
                    res.append((True, self.w3.codec.encode(['address'], [r])))
                except Exception:
                    res.append((False, b""))
        ...
        for raddr, pairs in by_resolver.items():
            resolver = self.w3.eth.contract(address=self.w3.to_checksum_address(raddr), abi=self.ens_resolver_abi)
            if use_multicall:
                calls = [(raddr, resolver.encodeABI(fn_name="name", args=[node])) for _, node in pairs]
                res2 = self._aggregate3(calls, allow_failure=True)
            else:
                res2 = []
                for _, node in pairs:
                    try:
                        nm = resolver.functions.name(node).call()
                        res2.append((True, self.w3.codec.encode(['string'], [nm])))
                    except Exception:
                        res2.append((False, b""))
    ...
    def batch_ens_forward(self, ens_names: List[str], use_multicall: bool=True) -> Dict[str, Optional[str]]:
        # 1) resolver(namehash(name))
        name_nodes = {n: self._namehash(n) for n in ens_names}
        if use_multicall:
            calls = [(self.ens_registry.address, self.ens_registry.encodeABI(fn_name="resolver", args=[node])) for node in name_nodes.values()]
            res = self._aggregate3(calls, allow_failure=True)
        else:
            res = []
            ...
            if use_multicall:
                ...
                for _, node in pairs:
                    try:
                        ...
                        res2.append((True, self.w3.codec.encode(['address'], [a])))
                    except Exception:
                        res2.append((False, b""))
            for (nm, _node), (ok, data) in zip(pairs, res2):
                if ok and data and len(data) >= 32:
                    try:
                        a = self.w3.codec.decode(['address'], data)[0]
                        out[nm] = self.w3.to_checksum_address(a) if int(a, 16) != 0 else None
                    except Exception:
                        out[nm] = None
        return out
    ...
    def multicall_fetch(
        ...
        *,
        ...
        # --- ENS ---
        if want_ens:
            out["ens_reverse"] = self.batch_ens_reverse(wallets, use_multicall=use_multicall)
            if ens_names:
                out["ens_forward"] = self.batch_ens_forward(ens_names, use_multicall=use_multicall)
